...
int main(int argc, char** argv) {
    ...
    cout<<"Converting to LUHN to find the final digit: ";
...
void tst2Lun(char *cc,int n){
    ...
    int counter=0;
    int cccS=n-1;
    char *ccc=new char[cccS];
    
    for (int loop=0;loop<tests;loop++){
        char chkDig=cc[n-1]-48;
        int temp=0, flip=0;
        for (int i=0;i<cccS;i++){
            ccc[i]=cc[i];
        }
        
        flip=rand()%cccS; //Determine which digit to flip
        ccc[flip]=rand()%9+48; //replace random digit
        flip=rand()%cccS; //Determine which 2nd digit to flip
        ccc[flip]=rand()%9+48; //replace 2nd random digit
        for (int i=n-2;i>=0;i-=2){ //Double every 2nd digit
            ccc[i]=(ccc[i]-48)*2;
            if (ccc[i]>9){ //if doubled digit > 9 then add the two digits together
                ccc[i]=(ccc[i]%10+ccc[i]/10);
            }
            ccc[i]=ccc[i]+48; //restore value in array to the numerical value not int value
        }
        //Sum all of the digits
        for (int i=0;i<=cccS-1;i++){
            temp+=(ccc[i]-48);
        }
        ...
        if (temp==chkDig) counter++; //Luhn is valid if sum of digits mod 10 is 0
    }
    ...
    for (int loop=0;loop<tests;loop++){ //Test LUHN after transposing 2 digits
        ...
        dig1=rand()%cccS; //Determine transposition spot 1
        dig2=rand()%cccS; //Determine transposition spot 2
        char swap='\0';
        swap=ccc[dig1];
        ccc[dig1]=ccc[dig2];
        ccc[dig2]=swap;
...
bool valLuhn(char *cc,int n){
    ...
    //test to see if Luhn is valid
    if (temp==chkDig) result=true; //Luhn is valid if sum of digits mod 10 is 0
