...
void Game::mapClr() { //reset map to all ocean tiles
    for (int i=0;i<rows;i++){
        for (int j=0;j<cols;j++){
            player.guess[i][j]='~';
            player.ship[i][j]='~';
...
bool Game::okShot(Guess shot) { //Check to see if shot is valid (hit, miss, on map, not duplicate)
    bool temp=false;
    if (player.ship[shot.getRow()][shot.getCol()]=='S') {
        temp=true;
        player.ship[shot.getRow()][shot.getCol()]='H';
        player.hits++;
        player.guesses++;
    } else if (player.ship[shot.getRow()][shot.getCol()]=='~') {
        temp=true;
        player.ship[shot.getRow()][shot.getCol()]='o';
        player.guesses++;
    } else {
        temp=false;
    }
...
ostream &operator<<(ostream &strm, const Game &obj)
{
    ...
    strm << BGBLACK << UNDERLINE << "         " << WHITE << "S" << " SHIP                        " << RESET << endl;
