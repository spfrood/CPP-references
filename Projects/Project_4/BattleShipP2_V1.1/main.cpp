...
void resHum(Player p1, Player p2, int rows){
    ...
    for (int i=0;i<rows;i++){ //for loop to loop through rows
        for (int j=0;j<COLS;j++){ //for loop to loop through columns
            if (p1.guess[i][j]!=0){ //each non 0 value is a guess 
    ...
    do { //begin do loop
        cin.clear();
        cout<<"Beginning round: "<<p1.guesses+1<<endl; //output round number (guesses so far plus 1)
        ...
        cout<<"Player 1 hits: "<<p1.hits<<" guesses: "<<p1.guesses<<endl; //output player 1 hits and guesses
        cout<<"Player 2 hits: "<<p2.hits<<" guesses: "<<p1.guesses<<endl; //output player 2 hits and guesses
        cout<<"<Press enter to continue, 'N' or 'n' to save and exit>"<<endl; //notify of pause
        cin.ignore(256, '\n'); //clear buffer
        getline (cin, pauser); //pause until enter pressed
    } while (p1.hits<14 && p2.hits<14 && pauser!="n" && pauser!="N"); //repeat loop until a player gets 14 hits or saves game
    if (pauser=="n" || pauser=="N") savGame(p1.ship, p1.guess, p2.ship, p2.guess, rows); //if game ends because of save then call save function
...
void resComp(Player p1, Player p2, int rows){
    ...
    do { //begin do loop
        ...
        p1.hits+=entGues(p1.guess, p2.ship, rows); //call guess function, increment player 1 hits if guess hits
        p1.guesses++; //increment number of player 1 guesses
        cout<<"Computer guessing now:"<<endl;
        p2.hits+=comGues(p2.guess, p1.ship, rows, p2.guesses); //call comp guess function increment player 2 hits if guess hits
        p2.guesses++; //increment player 2 guesses
        cout<<"Current Status: "<<endl;
        cout<<"Player 1 hits: "<<p1.hits<<" guesses: "<<p1.guesses<<endl; //output player 1 hits and guesses
        cout<<"Player 2 hits: "<<p2.hits<<" guesses: "<<p2.guesses<<endl; //output player 2 hits and guesses
        cout<<"<Press enter to continue, 'N' or 'n' to save and exit>"<<endl; //notify of pause
        cin.ignore(256, '\n'); //clear buffer
        getline (cin, pauser); //pause until enter pressed or save char entered
    } while (p1.hits<14 && p2.hits<14 && pauser!="n" && pauser!="N"); //loop until 14 hits scored by player or game saved
    if (pauser=="n" || pauser=="N") savGame(p1.ship, p1.guess, p2.ship, p2.guess, rows); //call game save function if necessary
...
void resGame(Player p1, Player p2, int rows){
    ...
    chooser = val::inChar('c', 'C', 'h', 'H'); //choose computer or human opponent
...
void savGame(int p1ship[][COLS], int p1guess[][COLS], int p2ship[][COLS], int p2guess[][COLS], int rows){
    ...
    if (fil.fail()) {
        ...
        fil.close();
        cout<<"BONES file not found! Creating BONES file!"<<endl;
...
int entGues(int pGuess[][COLS], int opShip[][COLS], int rows){
    ...
    while (!shipSet){ //loop until successful guess
        cout<<"Enter Column (A through J)"<<endl;
        cin>>putCol; //enter column
        while (cin.fail() || !isalpha(putCol) || (putCol>74 && putCol<=90) || (putCol>106 && putCol<=122)){ //loop to validate that input was valid and in range
            cin.clear(); //clear cin error flags
            cin.ignore(256, '\n'); //clear buffer
            cout<<"CHEAT ACTIVATED! SHOWING COMPUTER SHIP LOCATIONS!"<<endl; 
            disShip(opShip, rows); //activate cheat if out of range data input
            cout<<"You must enter a letter A to J..."<<endl; 
        ...
        cout<<"Enter Row (0 to 9)"<<endl;
        putRow = val::inNum(0, 9); //input row number for guess 
...
int comGues(int pGuess[][COLS], int opShip[][COLS], int rows, int gues){
    ...
    while (!shipSet){
        if (gues>0 && gues%6==0){ //if total guesses > 0 and guesses%11==0 then give comp a free hit
            ...
            cout<<"Computer position already guessed!"<<endl;
...
void pGameH(Player p1, Player p2, int rows) {
    ...
    do { //begin do loop
        ...
        p1.hits+=entGues(p1.guess, p2.ship, rows); //call function for player 1 to enter a guess - increment hits if needed
        ...
        p2.hits+=entGues(p2.guess, p1.ship, rows); //call function for player 2 to enter a guess - increment hits if needed
        ...
        cout<<"Current Game Status:"<<endl;
        cout<<"Player 1 hits: "<<p1.hits<<" guesses: "<<p1.guesses<<endl; //output player 1 hits, player 1 guesses
        cout<<"Player 2 hits: "<<p2.hits<<" guesses: "<<p2.guesses<<endl; //output player 2 hits, player 2 guesses
        cout<<"<Press enter to continue, 'N' or 'n' to save and exit>"<<endl; //notify of pause
        cin.ignore(256, '\n'); //clear buffer
        getline (cin, pauser); //pause until enter key pressed
    } while (p1.hits<14 && p2.hits<14 && pauser!="n" && pauser!="N"); //loop until player has 14 hits or game saved
    if (pauser=="n" || pauser=="N") savGame(p1.ship, p1.guess, p2.ship, p2.guess, rows); //save game if required
    cout<<"Exiting Game!"<<endl;
}
...
void pGameC(Player p1, Player p2, int rows) {
    ...
    do { //begin do loop
        ...
        p1.guesses++; //increment player 1 guess count
        cout<<"Computer guessing now:"<<endl;
        p2.hits+=comGues(p2.guess, p1.ship, rows, p2.guesses); //call function for computer guess - increment hits if needed
        p2.guesses++; //increment player 2 guess count
        cout<<"Current Status: "<<endl;
        disGame(p1.guess, p1.ship, rows); //call function to display player 1 game data
        cout<<"Player 1 hits: "<<p1.hits<<" guesses: "<<p1.guesses<<endl; //output player 1 hits and player 1 guesses
        cout<<"Player 2 hits: "<<p2.hits<<" guesses: "<<p1.guesses<<endl; //output player 2 hits and player 2 guesses
        cout<<"<Press enter to continue, 'N' or 'n' to save and exit>"<<endl; //notify of pause
        cin.ignore(256, '\n'); //clear buffer
        getline (cin, pauser); //pause until enter pressed 
    } while (p1.hits<14 && p2.hits<14 && pauser!="n" && pauser!="N"); //loop until player has 14 hits or saves game
    if (pauser=="n" || pauser=="N") savGame(p1.ship, p1.guess, p2.ship, p2.guess, rows); //save game if necessary
    cout<<"Exiting Game!"<<endl;
}
        ...
        do { //begin do loop
            ...
            if (verHor=='v'){ //if ship placement vertical
                if (!rngFind(ship, rows, size, putRow, intCol, verHor)){ //check to see ship will fit or overlap
                    ...
                    cout<<"Invalid location! Trying again!"<<endl;
                } else { //otherwise 
    ...
    for (int i=0;i<rows;i++){ //loop through rows
        ...
        for (int j=0;j<COLS;j++){ //loop through columns
            if (ship[i][j]==0){ //if no ship present
                cout<<BGBLUE<<CYAN<<"~"<<RESET; //output cyan 'O'
            }
            else if (ship[i][j]==1){ //if undamaged ship present
                cout<<BGWHITE<<BOLDBLUE<<"X"<<RESET; //output black 'X'
                ...
                cout<<BGWHITE<<MAGENTA<<"o"<<RESET; //output magenta 'o'
            }
        }
        cout<<endl;
    }
...
void disGame(int guess[][COLS], int ship[][COLS], int rows){
    cout<<"Your GUESSES   Ocean="<<BGBLUE<<CYAN<<"~"<<RESET<<"  Miss="<<BGWHITE<<MAGENTA<<"o"<<RESET
    ...
    for (int i=0;i<rows;i++){ //loop through rows
        ...
        for (int j=0;j<COLS;j++){ //loop through columns
            if (guess[i][j]==0){ //if guess array value zero
                ...
                cout<<BGWHITE<<MAGENTA<<"o"<<RESET; //output black 'M'
            }
    ...
    cout<<endl;
    cout<<"Your SHIPS   Ocean="<<BGBLUE<<CYAN<<"~"<<RESET<<"  Enemy Miss="<<BGCYAN<<MAGENTA<<"o"
            <<RESET<<"  Hit Ship="<<RED<<INVERSE<<"H"<<RESET<<"  Ship Position="
            <<BGWHITE<<BOLDBLUE<<"X"<<RESET<<endl; //output map legend
    cout<<"   ABCDEFGHIJ"<<endl;
    for (int i=0;i<rows;i++){ //loop through rows
        cout<<setw(3)<<left<<i; //output row number
        for (int j=0;j<COLS;j++){ //loop through columns
            if (ship[i][j]==0){ //if array value 0
                ...
                cout<<BGBLUE<<MAGENTA<<"o"<<RESET; //output magenta 'o'
            }
        }
