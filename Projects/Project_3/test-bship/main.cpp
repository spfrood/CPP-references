...
void savGame(int p1ship[][COLS], int p1guess[][COLS], int p2ship[][COLS], int p2guess[][COLS], int rows){
    ...
    while (line!=gName[temp] && temp<count){ //check to see if file name is duplicate
        temp++;
        cout<<" "<<temp;
    }
    cout<<endl;
    if (temp==count) {
        cout<<"gName[temp-1] = "<<gName[temp-1]<<endl;
        gName[temp-1]=line; //if temp == count then no duplicate names
    }
    else { //otherwise 
        cout<<"Overwriting existing game name: "<<gName[temp]<<endl; 
        gName[temp]=line; //overwrite new savefile name over array element with same name
    }
