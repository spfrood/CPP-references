...
//Function Prototypes
void prnChck(EmpData, ComData); //Function to print the paycheck
EmpData *addLine(EmpData *, EmpData, int); //Function to add additional entry to array
void pEmpCon(EmpData *, int); //display contents of array (all workers and checks cut)
...
int main(int argc, char** argv) {
    ...
    while (worker.hrsWrkd >0 && worker.payRate>0) { //keep looping until negative pay or hours value
        prnChck(worker, company); //print the employee's check
        cout<<endl;
        worList=addLine(worList, worker, size); //Add a line to the array and write the employee data to array
    ...
    pEmpCon(worList, size); //display contents of array (all workers and checks cut)
...
EmpData *addLine(EmpData *a, EmpData e, int size) {
